...
export default async function NewProductPage() {
  ...
  // Lista combinada (API + overlay) — incluye familias creadas en settings que aún no tienen productos.
  const families = await listAllFamilies().catch(() => []);
  ...
  return (
    <div className="px-5 md:px-10 py-8 space-y-8 max-w-3xl">
      ...
      <ProductCreateForm families={families.map((f) => f.slug)} />
    </div>
  );
}
